...
const linkSelectors = {
  ...
  self: 'atom:link[@rel="self" and @type="application/atom+xml"]', // only available when entry was received from feed
...
module.exports = (stringOrXMLDoc, isEntryNode) => {
  ...
  const entryNode = isEntryNode ? xmlDoc : xpathSelect('atom:entry', xmlDoc, true);
  const post = {
    ...
    status: xpathSelect('string(snx:moderation/@status)', entryNode, true),
